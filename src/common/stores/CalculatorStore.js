...
var CalculatorStore = assign({}, EventEmitter.prototype, {
  getDisplayScreen: function () {
    if (_displayScreen.toString().length >= _totalNumberOfDigits) {
      ...
      return parseFloat(_displayScreen).toExponential(
        _exponentialNumberOfDigits
      );
    }
  ...
  getDisplayFormulae: function () {
    ...
    _displayFormulae = _displayFormulae
      .reverse()
...
function processNumberKeyPressed(keyType, keyValue) {
  ...
  if (
    _numberKeyPressedBuffer.length === 2 &&
    _numberKeyPressedBuffer[0] === "-" &&
    _numberKeyPressedBuffer[1] === "0"
  ) {
  ...
  if (keyValue === ".") {
    if (~_numberKeyPressedBuffer.indexOf(".")) {
      ...
      if (
        !_numberKeyPressedBuffer.length ||
        (_numberKeyPressedBuffer.length === 1 &&
          _numberKeyPressedBuffer[0] === "-")
      ) {
...
function processCalculation() {
  if (_numbersFromBuffer.length === 2) {
    ...
    if (splitDisplay.length == 2) {
      var fixedTo =
        _totalNumberOfDigits - 2 - calculation.toString().split(".")[0].length;
      if (fixedTo < 0) {
        fixedTo = 0;
      }
    ...
    _displayNumber = converter.toWords(calculation).toString();
    if (calculation == "Error") {
      _numbersFromBuffer = [];
    } else {
      if (_numbersFromBuffer[0].toString().length >= _totalNumberOfDigits) {
        _numbersFromBuffer[0] = _numbersFromBuffer[0].toExponential(
          _exponentialNumberOfDigits
        );
      }
      if (_numbersFromBuffer[1].toString().length >= _totalNumberOfDigits) {
        _numbersFromBuffer[1] = _numbersFromBuffer[1].toExponential(
          _exponentialNumberOfDigits
        );
      }
      _displayFormulae.push({
        ...
        literal:
          "" +
          _numbersFromBuffer[0].toString() +
          " " +
          _symbolKeyTyped +
          " " +
          _numbersFromBuffer[1].toString(),
...
function processFormula(formula) {
  ...
  if (
    _displayFormulae[_displayFormulae.length - 1].literal !== formula.literal
  ) {
